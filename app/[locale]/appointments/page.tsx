...
export async function generateMetadata({
  ...
  return getLocalePageMetadata(params, 'appointments', 'appointmentsTitle', 'appointmentsDescription');
}
        ...
        <div className="mb-8">
          ...
          <div className="mt-2 flex w-full min-w-0 flex-row flex-wrap items-center justify-between gap-3 md:gap-4">
            ...
            <Link
              href="/bookings"
              className={cn(
                'inline-flex shrink-0 items-center justify-center rounded border-2 border-foreground bg-transparent px-5 py-2.5 text-xs font-semibold uppercase tracking-wide text-foreground transition hover:bg-brand-cream/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30',
              )}
            >
              {tNav('makeBooking')}
            </Link>
          </div>
          <p className="mt-3 max-w-2xl text-sm text-foreground/75">{t('intro')}</p>
        </div>
        <Suspense
          fallback={
