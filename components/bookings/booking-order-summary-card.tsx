...
export type BookingOrderSummaryCardProps = {
  ...
  /** When set, each line shows a remove control that calls this (parent shows confirm). */
...
export function BookingOrderSummaryCard({
  ...
  const lines = useMemo(() => {
    return orderedValidStyleIds.map((id) => byId.get(id)).filter((s): s is StyleResponse => Boolean(s));
  }, [orderedValidStyleIds, byId]);

  const totalCents = useMemo(() => {
    const withPrice = lines.map((s) => s.priceCents).filter((c): c is number => c != null);
    if (withPrice.length === 0) return null;
    return withPrice.reduce((a, b) => a + b, 0);
  }, [lines]);
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 px-1 pt-4">
        <div className="min-h-0 max-h-[min(36rem,65dvh)] flex-1 scroll-smooth overflow-y-auto overscroll-y-contain rounded-lg border border-border bg-muted/25 p-4 text-sm">
